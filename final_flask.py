...
def classification_model_metrics(model: str) -> dict:
    ...
    pol_df = pd.read_excel('model_KNN/dataset.xlsx')
    pol_df.drop_duplicates(inplace=True)

    label_encoder = LabelEncoder()
    pol_df["Пол"] = label_encoder.fit_transform(pol_df["Пол"])
    x = pol_df.drop(["Пол"], axis=1)
    y = pol_df["Пол"]
...
def get_sort():
    X_new = np.array([[float(request.args.get('rost')),
                       ...
                       float(request.args.get('razmer'))]])
    pred = loaded_model_knn.predict(X_new)
    pol = pol_df[pred[0]]
    return jsonify(sort=pol)
